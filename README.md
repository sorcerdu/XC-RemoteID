...
本项目仅用于 GB 46750-2025 广播式运行识别技术学习、协议研究和开发验证，不作为民用无人驾驶航空器运行识别合规设备提供，也不建议直接安装到实际无人机上用于运行活动。

运行识别属于民用无人驾驶航空器系统的安全与合规功能。若用于真实飞行、商业运营、登记审查、适航/合规验证或其他正式场景，应购买并使用具备相应资质和完整合规能力的厂商产品，或由具备能力的单位完成硬件安全、防篡改、网络式上报、测试验证和合规确认后再投入使用。

使用者基于本项目进行改装、测试或部署所产生的风险和责任，由使用者自行承担。
...
Release 提供各板型的完整固件（包含 bootloader、分区表和 app），刷写地址统一为 `0x0`。

| 文件名 | 适用板型 |
|--------|---------|
| `XC-RemoteID-esp32s3_devkit-<版本>.bin` | ESP32-S3 DevKitC-1 |
| `XC-RemoteID-esp32c3_supermini-<版本>.bin` | ESP32-C3 Super Mini |
| `XC-RemoteID-esp32c3_devkit-<版本>.bin` | ESP32-C3 DevKit |
| `XC-RemoteID-xiao_esp32c3-<版本>.bin` | XIAO ESP32-C3 |
...
4. 点击 **Add File**，上传下载好的 `.bin` 文件，地址填 `0x0`
...
esptool.py --port /dev/ttyUSB0 --baud 921600 \
  write_flash 0x0 XC-RemoteID-esp32c3_supermini-v0.1.0.bin
```
